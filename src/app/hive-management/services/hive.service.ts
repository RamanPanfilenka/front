...
export class HiveService {
  ...
  private urlHiveController = 'http://localhost:56952/api/hives';
  ...
  addHive(hive: Hive): Observable<Hive> {
    return this.http.post<Hive>(`${this.url}${hive.id}`, hive);
  }
  ...
  setHiveStatus(hiveId: number, deletedStatus: boolean): Observable<Object> {
    return this.http.put(`${this.url}${hiveId}/status/${deletedStatus}`, Object);
  }
}
